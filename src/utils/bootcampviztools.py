...
def trivariant_analysis_dispersion(df, directora, num1, num2, tamano_puntos = 20):
    col_directora = directora #variable principal que divide los datos en grupos 
    col_1 = num1 #variable numerica
    col_2 = num2 # ariable numerica
    diccionario_multivariante = {}

    for valor in df[col_directora].unique():
        diccionario_multivariante[valor] = df.loc[df[col_directora] == valor,[col_2,col_1]] #OJO al orden

    for valor,df_datos in diccionario_multivariante.items():
        print(f"Respuesta {valor}:")
        grafico_dispersion_con_correlacion(df_datos,col_2,col_1, tamano_puntos = tamano_puntos)
